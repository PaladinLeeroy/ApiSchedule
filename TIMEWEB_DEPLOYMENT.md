...
```bash
git add .
git commit -m "Fix Timeweb port detection issue"
...
- **Версия Python**: `3.9`
- **Команда запуска**: `python timeweb_app.py` ⚠️ **ВАЖНО!**
- **Порт**: `8000`
...
## 🔧 Устранение проблем с портом

### Если возникает ошибка "Unable to detect application port":

#### Вариант 1: Использовать uvicorn напрямую
Измените команду запуска на:
```
uvicorn app.main:app --host 0.0.0.0 --port 8000
```

#### Вариант 2: Использовать gunicorn с явным портом
Измените команду запуска на:
```
gunicorn app.main:app --bind 0.0.0.0:8000 --worker-class uvicorn.workers.UvicornWorker
```

#### Вариант 3: Использовать специальный файл (рекомендуется)
Оставьте команду запуска: `python timeweb_app.py`

### Проверка работы приложения:

1. **Проверьте логи** в панели Timeweb
2. **Убедитесь, что приложение слушает порт 8000**
